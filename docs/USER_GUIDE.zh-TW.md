...
> 本說明適用於目前 OpenCam 介面。Windows 與 macOS 的主要流程相同；平台差異會在各章節中特別標示。
...
不確定「螢幕 1／2／3…」分別是哪一台時，先選擇「指定螢幕」，再按「辨識螢幕」。OpenCam 會短暫在每台已連接的顯示器上顯示對應編號，並特別標示目前選中的螢幕；不論連接幾台都依清單編號顯示。若顯示器位置或縮放設定無法可靠對應，OpenCam 會略過不確定的標籤並提示，避免標錯螢幕。辨識標籤不會進入錄影，開始準備錄影時也會關閉。
