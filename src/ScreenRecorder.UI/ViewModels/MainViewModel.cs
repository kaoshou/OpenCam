...
public partial class MainViewModel : ObservableObject
{
    ...
    internal void SetDocumentationRecordingAudioActivity()
    {
        if (!_forScreenshot)
        {
            return;
        ...
        double[] systemLevels =
        [
            0.18, 0.27, 0.42, 0.31, 0.56, 0.72, 0.48, 0.36,
            0.64, 0.82, 0.59, 0.41, 0.29, 0.52, 0.76, 0.61,
            0.38, 0.24, 0.46, 0.69, 0.54, 0.33, 0.21, 0.44
        ];
        double[] microphoneLevels =
        [
            0.12, 0.21, 0.35, 0.58, 0.43, 0.28, 0.17, 0.39,
            0.66, 0.51, 0.32, 0.23, 0.47, 0.73, 0.62, 0.37,
            0.19, 0.31, 0.55, 0.78, 0.57, 0.34, 0.25, 0.49
        ];

        for (var i = 0; i < systemLevels.Length; i++)
        {
            ApplyAudioLevels(new AudioLevelsSnapshot(
                new AudioSourceLevel(AudioMeterState.Live, systemLevels[i], systemLevels[i]),
                new AudioSourceLevel(AudioMeterState.Live, microphoneLevels[i], microphoneLevels[i])));
        }
    }
