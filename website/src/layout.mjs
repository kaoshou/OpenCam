...
<header class="site-header"><nav class="nav-wrap" aria-label="${language === 'zh-TW' ? '主要導覽' : 'Main navigation'}"><a class="brand" href="${home}"><img src="${assetPath('app_icon.png')}" alt="" width="32" height="32">OpenCam</a><div class="nav-links"><a href="${home}">${language === 'zh-TW' ? '首頁' : 'Home'}</a><a href="${guide}">${guideLabel}</a><a class="language-switch" href="${otherRoute}" data-language="${other}">${other === 'zh-TW' ? '繁體中文' : 'English'}</a></div></nav></header>
<main id="main">${body}</main>
<footer class="site-footer"><div class="container"><span>© OpenCam</span><a href="https://github.com/kaoshou/OpenCam">GitHub</a><a href="${guide}">${guideLabel}</a></div></footer></body></html>`;
}

export function renderEntry() {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>OpenCam — Choose a language</title><link rel="stylesheet" href="${assetPath('site.css')}"><link rel="icon" href="${assetPath('app_icon.png')}"><script src="${assetPath('site.js')}" defer></script></head><body><main id="main" class="language-entry"><img src="${assetPath('app_icon.png')}" alt="" width="72" height="72"><h1>OpenCam</h1><p>Choose a language / 選擇語言</p><p><a href="${routeFor('zh-TW', 'home')}" data-language="zh-TW">繁體中文</a> <a href="${routeFor('en-US', 'home')}" data-language="en-US">English</a></p></main></body></html>`;
}
