...
test('desktop releases are gated by tests and reproducible FFmpeg inputs', async () => {
  ...
  assert.doesNotMatch(yaml, /BtbN\/FFmpeg-Builds\/releases\/download\/latest\//);
  assert.ok(yaml.includes(pinnedUrl));
  assert.ok(yaml.includes(sha256));

  const hashCheck = yaml.indexOf('Get-FileHash -Algorithm SHA256');
  const extraction = yaml.indexOf('Expand-Archive');
  assert.ok(hashCheck >= 0 && extraction > hashCheck, 'checksum must be verified before extraction');
  assert.match(yaml, /Test-Path[^\n]*ffmpeg-n8\.1\.3-win64-gpl-8\.1\\bin\\ffmpeg\.exe/);
